            ...
            <Slide class="" v-for="(serv, index) of serviciosRecientes" :key="index" style="height: 380px;">
              ...
              >
                <img style="object-fit: cover; height: 300px;"
                     :src="getImageUrl(serv.serv_imagen)"
                     alt="img"
                />
                <h4>{{ serv.serv_nombre }}</h4> 
              </router-link>  
            </Slide>
            <template #addons>
              <Pagination />
            ...
            <div class="blog-area pd-top-120 pd-bottom-120">
              <div class="container paused">
                ...
                <div v-else-if="servicios && servicios.length === 0" class="col-12 text-center py-5">
                  ...
                  <p class="text-muted mb-4">Pronto se agregarán nuevos servicios de la carrera.</p>
                ...
                <div v-else-if="searchGet" class="">
                  <div v-if="serviciosBusqueda.length == 0" class="col-12 text-center py-5">
                    ...
                    <p class="text-muted">No hay servicios que coincidan con "{{ search }}".</p>
                  ...
                  <div v-else class="row justify-content-center">
                    <div class="col-12">
                      <h3>{{ serviciosBusqueda.length }} resultado(s) encontrado(s)</h3>
                      <hr />
                    </div>
                    <div class="col-lg-5 col-md-6 zoom" 
                         v-for="(serv, index) of serviciosBusqueda"
                         :key="index">
                      <div class="solid-service-box">
                        <router-link
                          ...
                          @click="$store.commit('clickLink')"
                        >
                          <img :src="getImageUrl(serv.serv_imagen)"
                               alt="img"
                               width="300"
                          />
                        </router-link> 
                        <h5>{{ formatDate(serv.serv_registro) }}</h5>
                        <p v-if="serv.serv_nro_celular && serv.serv_nro_celular !== 2147483647">
...
export default {
  ...
  computed: {
    ...
    serviciosRecientes() {
      return this.servicios?.slice(0, 5) || [];
    },
    ...
    serviciosBusqueda() {
      ...
      return (this.servicios || []).filter(serv => 
        serv.serv_nombre?.toLowerCase().includes(term) ||
        serv.serv_descripcion?.toLowerCase().includes(term)
      );
    }
  },
